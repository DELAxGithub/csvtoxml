...
入力:
  046_土門_whisper.csv (Speaker Name, Start Time, End Time, Text)
  046_松田_whisper.csv (同上)

出力:
  046_whisper_merged.csv (Speaker Name, イン点, アウト点, 文字起こし, 色選択)
    - 両ピンマイクの発話を時間順にソート
    - HALLUCINATION_FILTERED 行は除外
    - 色: 土門=Lavender, 松田=Violet
    ★このファイルはTC厳密のまま。荒編とDaVinci流し込みはこれを読む。

  046_whisper_merged_clean.csv / _read.md / _cleanup_audit.json
...
# ===== エピソード固有設定 =====
EPISODE = "046"
EDIT_DIR = Path("/Users/delaxpro/Dropbox/プラッと/01_プラッと素材/046_松田土門/046edit")
INPUTS = [
    (EDIT_DIR / "046_土門_whisper.csv", "土門", "Lavender"),  # Tr1
    (EDIT_DIR / "046_松田_whisper.csv", "松田", "Violet"),    # Tr2
]
RUN_CLEANUP = True
