...
# ===== エピソード固有設定 =====
WAV_TR1 = "260531_001_Tr1.WAV"  # 石塚ピンマイク
WAV_TR2 = "260531_001_Tr2.WAV"  # 重田ピンマイク
# ===== VADパラメータ（基本変更不要）=====
FRAME_SEC = 0.1
MIN_SEG_SEC = 1.5
MAX_SEG_SEC = 20.0
...
def main():
    ...
    print("Detecting segments (Tr1=石塚)...")
    segs_ishizuka = detect_segments(rms1, rms2)
    print(f"  石塚 segments: {len(segs_ishizuka)}")
    print(f"  total speech: {sum(e-s for s,e in segs_ishizuka):.1f}s")

    print("Detecting segments (Tr2=重田)...")
    segs_shigeta = detect_segments(rms2, rms1)
    print(f"  重田 segments: {len(segs_shigeta)}")
    print(f"  total speech: {sum(e-s for s,e in segs_shigeta):.1f}s")
