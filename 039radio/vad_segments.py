...
# ===== エピソード固有設定 =====
WAV_TR1 = "260605_001_Tr1.WAV"  # 木崎ピンマイク
WAV_TR2 = "260605_001_Tr2.WAV"  # 朴ピンマイク
# ===== VADパラメータ（基本変更不要）=====
FRAME_SEC = 0.1
MIN_SEG_SEC = 1.5
MAX_SEG_SEC = 20.0
...
def main():
    ...
    print("Detecting segments (Tr1=木崎)...")
    segs_tr1 = detect_segments(rms1, rms2)
    print(f"  木崎 segments: {len(segs_tr1)}")
    print(f"  total speech: {sum(e-s for s,e in segs_tr1):.1f}s")

    print("Detecting segments (Tr2=朴)...")
    segs_tr2 = detect_segments(rms2, rms1)
    print(f"  朴 segments: {len(segs_tr2)}")
    print(f"  total speech: {sum(e-s for s,e in segs_tr2):.1f}s")
