...
使い方（コンソール）:
    exec(open("/Users/delaxstudio/src/70_プラッと/platto-automation/csvtoxml/037radio/csv_to_resolve_timeline.py", encoding="utf-8").read())
"""

import csv

# ===== 設定 =====
CSV_PATH = "/Users/delaxstudio/src/70_プラッと/platto-automation/csvtoxml/037radio/37__S.csv"
TIMELINE_NAME = "37__S_api"
FOLDER_NAME = "037"
TR1_CLIP = "260420_001_Tr1.WAV"
TR2_CLIP = "260420_001_Tr2.WAV"
CSV_FPS = 25
...
def write_companion_srt(rows, csv_fps, rec_offset_sec, srt_path, with_speaker=True):
    """build_blocks() と同じ写像で行ごとに SRT を吐く（block 集約せず行粒度）。
    数学的整合は scripts/srt_from_csv.py:project_to_timeline() と同一。
    片方を直したらもう片方も合わせること。"""
    rec_pos = rec_offset_sec
    block_first_in = None
    block_rec_start = None
    prev_color = None
    entries = []
    for r in rows:
        color = (r.get("色選択") or "").strip()
        try:
        ...
        srt_in = block_rec_start + row_offset
        srt_out = srt_in + (out_sec - in_sec)
        text = (r.get("文字起こし") or "").strip()
        speaker = (r.get("Speaker Name") or "").strip()
        if not text:
            rec_pos = block_rec_start + (out_sec - block_first_in)
            continue
        if with_speaker and speaker:
            text = f"[{speaker}] {text}"
        entries.append((srt_in, srt_out, text))
        rec_pos = block_rec_start + (out_sec - block_first_in)
    with open(srt_path, "w", encoding="utf-8") as f:
        for i, (a, b, t) in enumerate(entries, 1):
            if b <= a:
                b = a + 0.5
            f.write(f"{i}\n{_seconds_to_srt_tc(a)} --> {_seconds_to_srt_tc(b)}\n{t}\n\n")
    return len(entries)
...
def main():
    ...
    # 同じ写像でタイムライン TC 基準の SRT を併出
    srt_path = CSV_PATH.rsplit(".", 1)[0] + ".srt"
    n_srt = write_companion_srt(rows, CSV_FPS, rec_offset_sec, srt_path)
    print(f"SRT: {n_srt} entries → {srt_path}")
    print("DONE")
